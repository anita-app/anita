...
export const ProjectGridListElement: React.FC<IProjectGridListElementProps> = (props) => {
  ...
  return (
    <div key={props.element.id} className="inline-block h-min cursor-pointer" onClick={handleClick}>
      <div className="w-full bg-white rounded-lg shadow flex items-center justify-between p-6 space-x-6">
        <div className="flex-1 relative">
          <div className="flex items-center space-x-3">
            <h3 className="text-gray-900 text-sm font-medium truncate">{props.element[props.titleKey]}</h3>
          </div>
          {props.descriptionKey && props.descriptionKey !== props.titleKey && props.element[props.descriptionKey] && (
            <>
              <p className="mt-1 text-gray-500 text-sm max-h-96">{shortenDescription(props.element[props.descriptionKey])}</p>
