...
export class Section implements ISection {
  ...
  public getAllElements = async (): Promise<Array<SectionElement>> => {
    return dbInstances[this.projectId].callSelector<SectionElement>(this.id).multiple()
  }

  public getElementById = (id: string): Promise<SectionElement> => {
  ...
  public getFirstFieldOfType = (types: Array<FORM_COMPONENTS_CODES>): FormFieldsModel<SupportedFormsTypes> | undefined => {
    return this.formModel.find(formEle => types.includes(parseInt(formEle.componentCode as unknown as string)))
  }

  public getParentInfoFormEle = (): FormFieldsModel<SectionElement> => {
    return new ParentInfoFormEleBuilder(this.childOf, this.allSections).build()
  }
